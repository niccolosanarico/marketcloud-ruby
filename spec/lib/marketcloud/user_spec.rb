...
RSpec.describe Marketcloud::User do
	...
	describe 'a GET on a valid user' do
	  let(:user) { VCR.use_cassette('user') { Marketcloud::User.find(14516) }}
		...
		it 'answers to find with a valid user' do
		  expect(user.name).to eq "niccolo"
		end
	...
	describe 'a POST authenticating a user' do
		let(:auth_user) { VCR.use_cassette('user_authenticate') { Marketcloud::User.authenticate('prova@prova.it', 'provapw') }}
		it 'should return 200' do
			expect(auth_user.response.status).to eq 200
		end

		it 'should return a valid user' do
			expect(auth_user.email).to eq 'prova@prova.it'
		end
